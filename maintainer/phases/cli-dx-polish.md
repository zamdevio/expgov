...
1. **Per-command sections** in `help/index.ts` add:
   - **Examples** (3 lines max): real invocations using dogfood paths.
   - **Related:** cross-links (`diff` → `inventory`, `validate`; `timeline` → `trend`, `diff`).
   - **Next steps:** one line after each section (`Run expgov validate` after `inventory`).
2. **Workflow blocks** in `topic === 'all'`:
...
3. **Commander `--help`:** `configureCliHelp` post-process injects “See also” line from shared map.
4. **Error hints:** `printHelpHint` already suggests `--help`; add topic-specific hint from `ExportError.details.command`.

**Dependencies:** None.

**Complexity:** Low–medium (content-heavy).

**Risks:** Help length; keep `expgov help inventory` focused, aggregate workflows only in `help all`.
